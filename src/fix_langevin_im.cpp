...
FixLangevinIm::FixLangevinIm(LAMMPS *lmp, int narg, char **arg) :
    Fix(lmp, narg, arg), gfactor1(nullptr), gfactor2(nullptr), ratio(nullptr), tstr(nullptr),
    flangevin(nullptr), tforce(nullptr), franprev(nullptr), lv(nullptr), id_temp(nullptr),
    random(nullptr)
{
  ...
  while (iarg < narg) {
    if (::strcmp(arg[iarg], "angmom") == 0) {
      ...
      int const itype = utils::inumeric(FLERR, arg[iarg + 1], false, lmp);
      double const scale = utils::numeric(FLERR, arg[iarg + 2], false, lmp);
...
FixLangevinIm::~FixLangevinIm() noexcept(true)
{
  delete random;
  delete[] tstr;
  delete[] gfactor1;
...
int FixLangevinIm::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
  mask |= POST_FORCE_RESPA;
  if (tallyflag != 0) { mask |= END_OF_STEP; }
  return mask;
}

/* ---------------------------------------------------------------------- */

void FixLangevinIm::init()
{
  if ((oflag != 0) && (atom->omega_flag == 0)) {
    error->all(FLERR, "Fix langevin omega requires atom attribute omega");
  }
  if ((oflag != 0) && (atom->radius_flag == 0)) {
    error->all(FLERR, "Fix langevin omega requires atom attribute radius");
  }
  if ((ascale != 0.0) && (atom->ellipsoid_flag == 0)) {
  ...
  if (tstr != nullptr) {
    ...
    if (input->variable->equalstyle(tvar) != 0) {
      tstyle = EQUAL;
    } else if (input->variable->atomstyle(tvar) != 0) {
      tstyle = ATOM;
    } else {
  ...
  if (oflag != 0) {
    ...
    for (int i = 0; i < nlocal; ++i) {
      if (((mask[i] & groupbit) != 0) && (radius[i] == 0.0)) {
  ...
  if (ascale != 0.0) {
    ...
    if (avec == nullptr) { error->all(FLERR, "Fix langevin angmom requires atom style ellipsoid"); }
    ...
    for (int i = 0; i < nlocal; ++i) {
      if (((mask[i] & groupbit) != 0) && (ellipsoid[i] < 0)) {
  ...
  if (atom->rmass == nullptr) {
    for (int i = 1; i <= atom->ntypes; ++i) {
      gfactor1[i] = -atom->mass[i] / t_period / force->ftm2v / ratio[i];
      gfactor2[i] = ::sqrt(atom->mass[i] * 2.0 * force->boltz / t_period / update->dt /
                           force->mvv2e / ratio[i]) /
          force->ftm2v;
    }
  }

  if ((temperature != nullptr) && (temperature->tempbias != 0)) {
    tbiasflag = BIAS;
  } else {
    tbiasflag = NOBIAS;
  }
...
void FixLangevinIm::setup(int vflag)
{
...
void FixLangevinIm::post_force(int /*vflag*/)
{
  ...
  if (tstyle == ATOM) {
    if (tallyflag != 0) {
      if (tbiasflag == BIAS) {
        if (rmass != nullptr) {
          if (zeroflag != 0) {
            post_force_templated<1, 1, 1, 1, 1>();
          } else {
            post_force_templated<1, 1, 1, 1, 0>();
          }
        } else if (zeroflag != 0) {
          post_force_templated<1, 1, 1, 0, 1>();
        } else {
          post_force_templated<1, 1, 1, 0, 0>();
        }
      } else if (rmass != nullptr) {
        if (zeroflag != 0) {
          post_force_templated<1, 1, 0, 1, 1>();
        } else {
          post_force_templated<1, 1, 0, 1, 0>();
        }
      } else if (zeroflag != 0) {
        post_force_templated<1, 1, 0, 0, 1>();
      } else {
        post_force_templated<1, 1, 0, 0, 0>();
      }
    } else if (tbiasflag == BIAS) {
      if (rmass != nullptr) {
        if (zeroflag != 0) {
          post_force_templated<1, 0, 1, 1, 1>();
        } else {
          post_force_templated<1, 0, 1, 1, 0>();
        }
      } else if (zeroflag != 0) {
        post_force_templated<1, 0, 1, 0, 1>();
      } else {
        post_force_templated<1, 0, 1, 0, 0>();
      }
    } else if (rmass != nullptr) {
      if (zeroflag != 0) {
        post_force_templated<1, 0, 0, 1, 1>();
      } else {
        post_force_templated<1, 0, 0, 1, 0>();
      }
    } else if (zeroflag != 0) {
      post_force_templated<1, 0, 0, 0, 1>();
    } else {
      post_force_templated<1, 0, 0, 0, 0>();
    }
  } else if (tallyflag != 0) {
    if (tbiasflag == BIAS) {
      if (rmass != nullptr) {
        if (zeroflag != 0) {
          post_force_templated<0, 1, 1, 1, 1>();
        } else {
          post_force_templated<0, 1, 1, 1, 0>();
        }
      } else if (zeroflag != 0) {
        post_force_templated<0, 1, 1, 0, 1>();
      } else {
        post_force_templated<0, 1, 1, 0, 0>();
      }
    } else if (rmass != nullptr) {
      if (zeroflag != 0) {
        post_force_templated<0, 1, 0, 1, 1>();
      } else {
        post_force_templated<0, 1, 0, 1, 0>();
      }
    } else if (zeroflag != 0) {
      post_force_templated<0, 1, 0, 0, 1>();
    } else {
      post_force_templated<0, 1, 0, 0, 0>();
    }
  } else if (tbiasflag == BIAS) {
    if (rmass != nullptr) {
      if (zeroflag != 0) {
        post_force_templated<0, 0, 1, 1, 1>();
      } else {
        post_force_templated<0, 0, 1, 1, 0>();
      }
    } else if (zeroflag != 0) {
      post_force_templated<0, 0, 1, 0, 1>();
    } else {
      post_force_templated<0, 0, 1, 0, 0>();
    }
  } else if (rmass != nullptr) {
    if (zeroflag != 0) {
      post_force_templated<0, 0, 0, 1, 1>();
    } else {
      post_force_templated<0, 0, 0, 1, 0>();
    }
  } else if (zeroflag != 0) {
    post_force_templated<0, 0, 0, 0, 1>();
  } else {
    post_force_templated<0, 0, 0, 0, 0>();
  }
}

/* ---------------------------------------------------------------------- */

void FixLangevinIm::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixLangevinIm::post_force_templated()
{
  double gamma1{};
  double gamma2{};
  ...
  double const boltz = force->boltz;
  double const dt = update->dt;
  double const mvv2e = force->mvv2e;
  double const ftm2v = force->ftm2v;
  ...
  if (Tp_ZERO) {
    ...
    if (count == 0) { error->all(FLERR, "Cannot zero Langevin force of 0 atoms"); }
  }
  ...
  if (oflag != 0) { omega_thermostat(); }
  if (ascale != 0) { angmom_thermostat(); }
}
...
void FixLangevinIm::compute_target()
{
  const int *const mask = atom->mask;
  int const nlocal = atom->nlocal;
  ...
  if (tstyle == CONSTANT) {
    ...
    if (tstyle == EQUAL) {
      ...
      for (int i = 0; i < nlocal; ++i) {
        if (((mask[i] & groupbit) != 0) && (tforce[i] < 0.0)) {
          error->one(FLERR, "Fix langevin variable returned negative temperature");
        }
      }
    }
...
void FixLangevinIm::omega_thermostat()
{
  double gamma1{};
  double gamma2{};
  ...
  double const tendivthree = 10.0 / 3.0;
  double tran[3]{};
  double inertiaone{};
...
void FixLangevinIm::angmom_thermostat()
{
  double gamma1{};
  double gamma2{};
...
void FixLangevinIm::end_of_step()
{
  double **v = atom->v;
  const int *const mask = atom->mask;
  int const nlocal = atom->nlocal;
...
void FixLangevinIm::reset_target(double t_new)
{
...
int FixLangevinIm::modify_param(int narg, char **arg)
{
...
double FixLangevinIm::compute_scalar()
{
  if ((tallyflag == 0) || (flangevin_allocated == 0)) { return 0.0; }
...
void *FixLangevinIm::extract(const char *str, int &dim)
{
  dim = 0;
...
double FixLangevinIm::memory_usage()
{
  double bytes = 0.0;
  if (tallyflag != 0) { bytes += static_cast<double>(atom->nmax) * 3 * sizeof(double); }
  if (tforce != nullptr) { bytes += static_cast<double>(atom->nmax) * sizeof(double); }
  return bytes;
}
...
void FixLangevinIm::grow_arrays(int nmax)
{
...
void FixLangevinIm::copy_arrays(int i, int j, int /*delflag*/)
{
...
int FixLangevinIm::pack_exchange(int i, double *buf)
{
  int n = 0;
  buf[n++] = franprev[i][0];
  buf[n++] = franprev[i][1];
...
int FixLangevinIm::unpack_exchange(int nlocal, double *buf)
{
  int n = 0;
