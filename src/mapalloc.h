...
class Mgr {
 public:
  ...
  int64_t add(int64_t n) { return allocated += n; }
...
template <class T> class Alloc {
 public:
  ...
  Alloc(Alloc &&other) noexcept(true) :
      lmp(other.lmp), allocated(other.allocated), ncall(other.ncall)
  {
    other.allocated = 0;
    other.ncall = 0;
  }
  ...
  template <class U> Alloc(Alloc<U> &&other) noexcept(true) = delete;

  ~Alloc() noexcept(true)
  {
    if (allocated > 0) {
      lmp->error->warning(
          FLERR, "Destructing allocator with allocated memory: {} elements unfreed ({} bytes)",
          allocated, allocated * type_size);
    }
  }

  // return maximum number of elements that can be allocated
  size_type max_size() const throw() { return std::numeric_limits<size_type>::max() / type_size; }
...
template <class T1, class T2> bool operator==(const Alloc<T1> &, const Alloc<T2> &) throw()
{
  return true;
}
template <class T1, class T2> bool operator!=(const Alloc<T1> &, const Alloc<T2> &) throw()
