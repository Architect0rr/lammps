    ...
    : Fix(lmp, narg, arg)
{
  ...
  if (nevery < 1)
    error->all(FLERR, "nevery for cluster/dump nust be greater than 0");

  // Get the critical size
  size_cutoff = utils::numeric(FLERR, arg[4], true, lmp);
  if (size_cutoff < 1)
    error->all(FLERR, "size_cutoff for cluster/dump must be greater than 0");

  // Get cluster/size compute
  compute_cluster_size = lmp->modify->get_compute_by_id(arg[5]);
  if (compute_cluster_size == nullptr){
    error->all(FLERR, "cluster/dump: Cannot find compute of style 'cluster/size' with id: {}", arg[5]);
  }

  // Get cluster/temp compute
  compute_cluster_temp = lmp->modify->get_compute_by_id(arg[6]);
  if (compute_cluster_temp == nullptr){
    error->all(FLERR, "cluster/dump: Cannot find compute of style 'cluster/temp' with id: {}", arg[6]);
  }
  ...
  if (compute_supersaturation_mono == nullptr){
    error->all(FLERR, "cluster/dump: Cannot find compute of style 'supersaturation/mono' with id: {}", arg[7]);
  }
  ...
  if (compute_supersaturation_density == nullptr){
    error->all(FLERR, "cluster/dump: Cannot find compute of style 'supersaturation/density' with id: {}", arg[8]);
  }
